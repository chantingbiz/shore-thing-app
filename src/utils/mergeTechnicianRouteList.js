 ...
 * @param {Array<{ property_slug: string, name: string, address: string, technician_slug?: string }>} dbList
 * @returns {Array<{ slug: string, name: string, address: string }>}
 */
export function mergeStephenRouteListWithDb(dbList) {
  const map = new Map();
  for (const p of STEPHEN_PROPERTIES) {
    map.set(p.slug, { slug: p.slug, name: p.name, address: p.address });
  }
